...
public class UtilTime {
    ...
    public static String convertString(long time, int trim, TimeUnit type) {
        ...
        if (type == TimeUnit.DAYS) {
            return String.valueOf(UtilMath.trim(trim, (double)time / 8.64E7)) + " Days";
        }
        if (type == TimeUnit.HOURS) {
            return String.valueOf(UtilMath.trim(trim, (double)time / 3600000.0)) + " Hours";
        }
        if (type == TimeUnit.MINUTES) {
            return String.valueOf(UtilMath.trim(trim, (double)time / 60000.0)) + " Minutes";
        }
        if (type == TimeUnit.SECONDS) {
            return String.valueOf(UtilMath.trim(trim, (double)time / 1000.0)) + " Seconds";
        }
        return String.valueOf(UtilMath.trim(trim, time)) + " Milliseconds";
    }
